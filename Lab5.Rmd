...
#get number of people who scored higher than 3 sd
...
Confidence:90
```{r}
#get sample mean with n=25
#identify the p-value in the sampling distribution
...
#The proportion in #4 is lower because the scores away from the mean are less frequent than the #3 distribution.  The reduced variability and standard error in #4 are due to the increased sample size.
```
...
#get sd with n=25
#identify the p-value in the sampling distribution
sd(rnorm(25,mean=10,sd=20))

samplDistrb<-replicate(10000,sd(rnorm(25,10,20)))
hist(samplDistrb)
pnorm(q=15,
      mean=mean(samplDistrb),
      sd=sd(samplDistrb),
      lower.tail=TRUE
      )
```
